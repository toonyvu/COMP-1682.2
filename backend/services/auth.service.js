...
export async function signup(username, email, password) {
    ...
    const emailresult = await pool.query("SELECT 1 FROM users WHERE email = $1", [email]);
...
export async function findOrCreateExistingUser(profile) {
    const client = await pool.connect();
    try {
        await client.query("BEGIN");
        // Find if user already logged in with this federated account
        const existingFederatedUser = await client.query(`SELECT user_id FROM federated_credentials WHERE provider = $1 AND provider_id = $2`, [profile.provider, profile.id]);
        ...
        if (userResult.rows.length === 0) {
            const userCreateResult = await client.query(`INSERT INTO users (username, email, avatar_url)
        VALUES ($1, $2, $3) RETURNING *`, [profile.displayName, email, avatar]);
            const createdUserId = userCreateResult.rows[0].id;
            await client.query(`INSERT INTO federated_credentials (user_id, provider, provider_id)
        VALUES ($1, $2, $3)`, [createdUserId, profile.provider, profile.id]);
            await client.query("COMMIT");
            ...
            await client.query(`INSERT INTO federated_credentials (user_id, provider, provider_id)
        VALUES ($1, $2, $3)`, [existingUserId, profile.provider, profile.id]);
            await client.query("COMMIT");
...
export async function storeRefreshToken(userId, refreshToken) {
    const insertResult = await pool.query(`INSERT INTO refresh_tokens (user_id, token) VALUES ($1, $2) RETURNING ID`, [userId, refreshToken]);
    if (insertResult)
        return;
}
export async function getRefreshToken(refreshToken) {
    const getResult = await pool.query(`SELECT * FROM refresh_tokens WHERE token = $1`, [refreshToken]);
