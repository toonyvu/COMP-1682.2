...
export async function getAllMealkits(page, limit, week, search, ids) {
    ids;
    const offset = (page - 1) * limit;
    if (ids.length > 0) {
        const mealkitResult = await pool.query(`SELECT 
        t1.id AS recipe_id, 
        t1.name, 
        t1.description, 
        t1.avatar_url, 
      ...
      ORDER BY t2.id 
      LIMIT $4 
      OFFSET $5`, [week, `%${search}%`, ids, limit, offset]);
      ...
      `, [week, `%${search}%`, ids]);
        ...
        const MealitResult = await pool.query(`SELECT t1.id AS recipe_id, 
    t1.name, 
    t1.description, 
    t1.avatar_url, 
    t1.servings, 
    ...
    ORDER BY t2.id 
    LIMIT $3 
    OFFSET $4`, [week, `%${search}%`, limit, offset]);
        const countResult = await pool.query(`
      SELECT COUNT(*)
      FROM recipes t1
      INNER JOIN mealkits t2
...
export async function createMealkit(mealkitData) {
    ...
    const insertMealkitResult = await pool.query(`INSERT INTO mealkits(recipe_id, week_number, year, price, available_from, available_until, max_servings) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`, [
...
//# sourceMappingURL=mealkits.service.js.map