...
export async function getOrder(sessionId) {
    const orderResult = await pool.query(`SELECT * FROM orders WHERE stripe_session_id = $1`, [sessionId]);
    ...
    const itemsResult = await pool.query(`SELECT t1.*, t2.price, t3.name, t3.avatar_url
     FROM order_items t1
     JOIN mealkits t2 ON t1.mealkit_id = t2.id
     JOIN recipes t3 ON t2.recipe_id = t3.id
     WHERE t1.order_id = $1`, [order.id]);
    const paymentIntent = checkoutData.payment_intent;
...
export async function cancelOrders(orderId) {
...
export async function getAllOrders(userId) {
    ...
    try {
        await client.query("BEGIN");
        const ordersResult = await client.query(`SELECT
      o.*,
      COALESCE(
        json_agg(
          json_build_object(
...
export async function getAllOrdersAdmin(searchField, search, status, sort, order, page, limit) {
