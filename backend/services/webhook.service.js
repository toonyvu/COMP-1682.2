    ...
    switch (event.type) {
        case "checkout.session.completed": {
            ...
            if (session.mode === "payment") {
                ...
                const paymentMethodId = (await paymentIntent).payment_method;
                ...
                const existing = await pool.query(`SELECT id FROM orders WHERE stripe_session_id = $1`, [session.id]);
                ...
                try {
                    ...
                    while (existing) {
                        ...
                        const customerOrderResult = await client.query("SELECT id FROM orders WHERE cus_order_id = $1", [customerId]);
          ...
          `, [userId]);
                ...
                const existing = await pool.query("SELECT id FROM subscriptions WHERE stripe_subscription_id = $1", [subscription.id]);
                if (existing.rows.length > 0) {
                    ("Order already exists!");
                    return;
                }
