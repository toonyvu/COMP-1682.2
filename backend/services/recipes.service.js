...
export async function getRecipeDetails(id, mealkitId) {
    ...
    ("Fetching continues..");
    const stepsResult = await pool.query("SELECT * FROM recipe_steps WHERE recipe_id = $1 ORDER BY step_number ASC", [id]);
    const mealkitResult = await pool.query(`
    SELECT * FROM mealkits
    WHERE id = $1`, [mealkitId]);
    ...
    `, [id]);
    const ingredientsResult = await pool.query(`SELECT 
...
export async function createRecipeService(recipe, ingredients, steps) {
    ...
    try {
      ...
      `, [
        ...
        VALUES ($1, $2, $3) RETURNING *`, [recipeId, step.step_number, step.instruction]);
...
export async function getAllRecipesAdmin(page, limit, search) {
    const offset = (page - 1) * limit;
    if (!search?.trim()) {
        const recipeResult = await pool.query(`SELECT * FROM recipes ORDER BY id ASC LIMIT $1 OFFSET $2`, [limit, offset]);
    ...
    const recipeResult = await pool.query(`SELECT * FROM recipes WHERE name ILIKE $1 ORDER BY id ASC LIMIT $2 OFFSET $3`, [`%${search}%`, limit, offset]);
    const countResult = await pool.query(`SELECT COUNT(*) as count FROM recipes WHERE name ILIKE $1`, [`%${search}%`]);
    return {
        recipes: recipeResult.rows,
        total: countResult.rows[0].count,
    };
}
export async function getRecipeAdmin(id) {
...
export async function editRecipeService(recipe, ingredients, steps) {
    const client = await pool.connect();
    const tagIds = recipe.tags.map(Number);
    try {
        await client.query("BEGIN");
