import { getOrder, getAllOrders, getOrderDetails, getAllOrdersAdmin, updateOrderStatus, cancelOrders, } from "../services/orders.service.js";
...
export async function getOrderDetailsAdminController(req, res) {
    const { orderId } = req.params;
...
export async function getOrdersAdminController(req, res) {
    try {
        ...
        const orders = await getAllOrdersAdmin(searchField, search, status, sort, order, page, limit);
        return res.status(200).json(orders);
    }
    catch (err) {
        return res.status(500).json({ message: err.message });
